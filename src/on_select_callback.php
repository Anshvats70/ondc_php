...
class ONDCSelectCallback {
    ...
    private function extractSelectParameters($requestData) {
        ...
        $this->log("Extracted Select Parameters: " . json_encode($params, JSON_PRETTY_PRINT));
        
        return $params;
    }
    
    /**
     * Process selection and get detailed item information
     */
    private function processSelection($selectParams) {
        $this->log("Processing selection for " . count($selectParams['items']) . " items");
    ...
    /**
     * Generate ONDC compliant select response
     */
        ...
        $response = [
            ...
            'message' => [
                ...
                'order' => [
                    ...
                    'items' => $this->formatSelectItems($itemDetails['items']),
                    'fulfillments' => $this->generateFulfillments($itemDetails),
                    'payments' => $this->generatePayments($itemDetails),
                    'billing' => $this->generateBilling($itemDetails),
                    'quote' => $this->generateQuote($itemDetails)
                ]
            ]
        ];
    ...
    /**
     * Format items for select response
     */
    private function formatSelectItems($items) {
        $formattedItems = [];
     ...
     */
    private function generateFulfillments($itemDetails) {
        return [
            [
                'id' => 'fulfillment_001',
                'type' => 'Delivery',
     ...
     */
    private function generatePayments($itemDetails) {
        return [
            [
                'id' => 'payment_001',
                'type' => 'ON-ORDER',
                'collected_by' => 'BAP',
                'params' => [
                    'amount' => (string)$itemDetails['total_price'],
     ...
     */
    private function generateBilling($itemDetails) {
        return [
            'name' => 'Customer Name',
            'address' => [
                'locality' => 'Customer Address',
     ...
     */
    private function generateQuote($itemDetails) {
        $subtotal = $itemDetails['total_price'];
...
if (php_sapi_name() === 'cli') {
    ...
    echo "📋 Testing with sample select request...\n\n";
    $response = $callback->handleSelectRequest($sampleRequest);
    
    echo "\n🎯 Test completed! Check logs/on_select_callback.log for details.\n";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $callback = new ONDCSelectCallback();
        $response = $callback->handleSelectRequest($requestData);
